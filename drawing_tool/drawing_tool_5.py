...
class Ui(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.overlay_ax = self.figure.add_subplot(111, sharex=self.ax, sharey=self.ax, frame_on=False)
        ...
        self.overlay_img = None  # holds the overlay image
        self.path = []  # path of the line
        self.preview_circle = None
        
        self.timer = QTimer()
        self.timer.timeout.connect(self.addLines)
    ...
    def pushButton_load_img_clicked(self):
        ...
        if file_name:
            ...
            self.img = rescale(rgb2gray(self.img), 0.25)
            # create a transparent RGBA overlay image
            self.overlay_img = np.zeros((*self.img.shape[:2], 4), dtype=float)  # use img.shape[:2] in case the image is grayscale
            self.overlay_img[..., 3] = 0  # set alpha channel to 0 (transparent)
            self.ax.imshow(self.img)
            self.overlay_ax.imshow(self.overlay_img)  # initialize imshow
            self.canvas.draw()
    ...
    def mousePress(self, event):
        if event.button == 1 and (event.inaxes == self.ax or event.inaxes == self.overlay_ax):  # left mouse button
            self.timer.start(50)
    ...
    def addLines(self):
        ...
        binary_img = np.zeros(self.img.shape[:2], dtype=bool)
        ...
        selem = disk(self.spinBox_brush_size.value())
        dilated_img = morphology.binary_dilation(binary_img, selem)

        # reset overlay image
        self.overlay_img[dilated_img > 0] = [1, 1, 1, 1]  # set color and alpha for the lines

        # update only the changed pixels
        self.overlay_ax.images[0].set_data(self.overlay_img)
        self.canvas.draw()


    def mouseMove(self, event):
        # add a line preview when hovering and mouse is pressed
        if (event.inaxes == self.ax or event.inaxes == self.overlay_ax) and self.path:
            self.path.append((event.xdata, event.ydata))

            
            
        elif (event.inaxes == self.ax or event.inaxes == self.overlay_ax):
